...
model_data$harvested <- factor(model_data$harvested)
...
```{r}
model_data$pred_prob <- predict(model_full, type = "response")
...
## Residual Plot
```{r}
res <- residuals(model_full, type = "deviance")
fit <- fitted(model_full)
...
model_comp <- data.frame(
  Model = c("Sensory-only model", "Full model"),
  AIC = c(AIC(model_sensory), AIC(model_full))
)

model_comp %>%
  kable(
    caption = "Preliminary comparison of logistic regression models",
    align = "cc",
    booktabs = TRUE,
    digits = 2
  ) %>%
