...
coffeedata <- coffeedata %>%
  mutate(quality_bin = case_when(
      Qualityclass == "Good" ~ 1,
      Qualityclass == "Poor" ~ 0))
glimpse(coffeedata)

#add a new column to save the log transformation of category_two_defects
coffeedata$log_defects <- log(ifelse(coffeedata$category_two_defects==0, coffeedata$category_two_defects + 1,coffeedata$category_two_defects))

#check the type of all variables
str(coffeedata)

# convert harvested into factor
